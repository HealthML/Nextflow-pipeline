...
class PlinkLoader(VariantLoader):
    """
    Deprecated

    Loads variants (SNVs) and accompanying information from binary PLINK 1 files.

    Has methods to subset the data set based on variant ids and individual ids.
    Can retrieve genotypes of a specific region or by variant id.

    :param str path_to_plink_files_with_prefix: path to binary PLINK 1 file and accompanying files with prefix
    """
    ...
    def __init__(self, path_to_plink_files_with_prefix):
        ...
        # Save numerical index to access genotypes (bed file) later on
        self.bim['chrom'] = self.bim['chrom'].astype(str)
        self.bim['bim_nindex'] = self.bim.index
        self.fam['iid'] = self.fam['iid'].astype(str)
        # Save numerical index to access genotypes (bed file) later on
        self.fam['fam_nindex'] = self.fam.index
        self.fam = self.fam.rename(columns={'iid': 'iid'}) #?
        self.fam.set_index(keys='iid', inplace=True)
        # Use snp name as index
        self.bim.set_index(keys='snp', inplace=True)
    ...
    def update_variants(self, vids=None, coordinates=None, exclude=False):
        ...
        if vids is None and coordinates is None:
            ...
            if exclude:
                try:
                    self.bim = self.bim.drop(vids)
                except KeyError:
                    logging.warning(
                        'At least one variant (vid) that you tried to exclude does not exist in data set.')
                    self.bim = self.bim.drop(vids, errors='ignore')
            else:
                self.bim = self.bim.loc[vids]
        else:
            if exclude:
                try:
                    temp_bim = self.bim[(self.bim['chrom'] == coordinates['chrom']) &
                                        (self.bim['pos'] > coordinates['start']) &
                                        (self.bim['pos'] <= coordinates['end'])]
                    self.bim = self.bim.drop(temp_bim.index)
                except KeyError:
                    logging.warning('No variants lie within region that you want to exclude.')
            else:
                self.bim = self.bim[
    ...
    def genotypes_by_region(self, coordinates, return_pos=False):
        ...
        """
        # only keep genotypes/veps_index_bed for selected region (genotype info)
        # TODO: this is probably very slow for large bims, make faster!
        temp_bim = self.bim[(self.bim['chrom'] == coordinates['chrom']) & (self.bim['pos'] > coordinates['start'])
                            & (self.bim['pos'] <= coordinates['end'])]

        # If no SNPs found for a gene, continue with the next gene!
        if temp_bim.empty:
            logging.warning('No variants were found for the region {}.'.format(coordinates))
            if return_pos:
                return None, None, None
        ...
        # Only load SNPs of respective gene
        temp_geno = self.bed[temp_bim['bim_nindex'].values, :].compute()
        # Drop individuals for which not all covariate and phenotype information is available
        temp_geno = temp_geno[:, self.fam['fam_nindex'].values]

        temp_geno = temp_geno.T

        if return_pos:
            return temp_geno, temp_bim.index, temp_bim['pos']
        else:
            return temp_geno, temp_bim.index
    ...
    def get_vids(self):
        ...
        :rtype: pandas.Index
        """
        return self.bim.index

    def genotypes_by_id(self, vids, return_pos=False):
        """Retrieves genotypes for a set of variant ids (:attr:`vids`).
...
class EnsemblVEPLoader(AnnotationLoader):
    ...
    def _parse_location(self, loc):
        ...
        ##### version1 - coded originally in seak, error in coordinate format chr:start-end #####
        # chrom = loc[:, 0]
        # pos = loc[:, 1].astype(np.int32)  # one-based!

        ##### version2 - returns np.arrays for both coordiante formats but subsequent errors  in seak #####
        # chrom = np.asarray(loc.iloc[:,0])
        # loc = loc.iloc[:,1].str.split('-', expand=True)
        # start = np.asarray(loc.iloc[:,0].astype(np.int32))
        # end = np.asarray(loc.iloc[:,1])
        # pos = (start, end)
        ...
        loc = loc.iloc[:,1].str.split('-', expand=True)
        start = np.asarray(loc.iloc[:,0].astype(np.int32))
        # end: takes the value of end in format start-end, when it is originally np.nan (so format is start) then it takes the value start
        loc.iloc[:,1] = np.asarray(loc.iloc[:,1].astype(np.float))
        end = np.asarray(np.where(np.isnan(loc.iloc[:,1].values), loc.iloc[:,0].values, loc.iloc[:,1].values ), dtype=np.int32)
...
class SpliceAILoader(Hdf5Loader):
    """Loads variant annotations from HDF5 files accompanied by a TSV with variant information.
    ...
    def __init__(self, path_to_vep_bed, path_to_vep_hdf5, hdf5_key):
        """Constructor."""
        super().__init__(path_to_vep_bed, path_to_vep_hdf5, hdf5_key)

    def _read_veps_from_bed(self, path_to_vep_bed, from_janggu):
        """Loads veps from UCSC BED file."""
        col_names = ['chrom', 'start', 'end', 'name', 'gene']
        veps_index_df = pd.read_csv(path_to_vep_bed, sep='\t', header=None, usecols=[0, 1, 2, 3, 4],
                                    names=col_names,
                                    low_memory=False, dtype={'chrom': str, 'name': str, 'start': int, 'end': int})
        veps_index_df['veps_nindex'] = veps_index_df.index
        veps_index_df.set_index(keys='name', inplace=True)
        return veps_index_df
    ...
    def anno_by_id_and_gene(self, vids, coordinates, gene=None, shuffle=None):
        ...
        :rtype: numpy.ndarray
        """
        gene = coordinates['name'] if gene is None else gene
        try:
            veps_indices = self.veps_index_df[self.veps_index_df['gene'] == gene].loc[vids]['veps_nindex'].values
            veps = self.veps[veps_indices, :]
            ...
            logging.warning('No veps for gene or variants.')
            return None

    def get_vids_by_region(self, coordinates):
        """Return variant ids that lie within a genomic region.

        The order of the variants (columns, axis 1) matches that returned by :func:`get_vids`.
        If no variants lie in the requested region the method returns None.

        :param dict coordinates: a dictionary of genomic coordinates {"chr": str, "start": int, "end": int, "name": str}
        :return: names/vids of respective veps
        :rtype: pandas.Index
        """
        temp_veps_index_df = self.veps_index_df[(self.veps_index_df['chrom'] == coordinates['chrom']) &
                                                (self.veps_index_df['start'] >= coordinates['start']) &
        ...
        if temp_veps_index_df.empty:
            logging.warning('No veps were found for the region {}.'.format(coordinates))
            return None

        else:
            return temp_veps_index_df.index

    def get_vids_by_region_and_gene(self, coordinates, gene=None):
        """Return variant ids that lie within a genomic region.
        ...
        """
        gene = coordinates['name'] if gene is None else gene
        temp_veps_index_df = self.veps_index_df[(self.veps_index_df['gene'] == gene) &
                                                (self.veps_index_df['chrom'] == coordinates['chrom']) &
                                                (self.veps_index_df['start'] >= coordinates['start']) &
                                                (self.veps_index_df['end'] <= coordinates['end'])]

        if temp_veps_index_df.empty:
            logging.warning('No veps were found for the region and gene {}.'.format(coordinates))
            return None
...
class SpliceAIRegionLoader(RegionLoader):
    """Reads all regions from SpliceAI accompanying region file."""

    def __init__(self, path_to_regions_UCSC_BED, chrom_to_load, drop_non_numeric_chromosomes=False):
        self.regions = self._read_regions(path_to_regions_UCSC_BED, chrom_to_load, drop_non_numeric_chromosomes)

    def _read_regions(self, path_to_regions_UCSC_BED, chrom_to_load, drop_non_numeric_chromosomes):
        # TX_START=start
        # TX_END = end
        col_names_USCS_bed_file_reference_genome = ['name', 'chrom', 'strand', 'start', 'end', 'EXON_START', 'EXON_END']
        regions = pd.read_csv(path_to_regions_UCSC_BED, sep='\t', header=0, usecols=[0, 1, 2, 3, 4, 5, 6],
                              names=col_names_USCS_bed_file_reference_genome, low_memory=False,
                              dtype={'chrom': str, 'name': str, 'start': int, 'end': int})
        if drop_non_numeric_chromosomes:
            # Remove all not numeric chromosome values!
            filter_non_autosomal_chromosomes = regions['chrom'].apply(lambda x: x.isdigit())
            regions = regions[filter_non_autosomal_chromosomes]
