...
# Kernel functions
def diffscore_max(G, V):
    """Uses the largest absolute variant effect predictions (veps) per SNV scaled by a factor of 100 as linear weights.
...
def single_column_kernel(i):
    """Creates a kernel function with weights that correspond to a single column of the veps numpy.ndarray.
...
def elavl1_v1(G, V):
    """Scales the SNVs by the variant effect prediction (vep) of the transcription factor ELAVL1 scaled by a factor of 100 as linear weights.

    Linear weighted kernel.
    v1 refers to the trained convolutional neural network that was used to derive the respective veps.
    ...
    :rtype: numpy.ndarray
    """
    return single_column_kernel(10)(G, V)  # np.matmul(G, V)
...
def elavl1_v2(G, V):
    ...
    Linear weighted kernel.
    v2 refers to the trained convolutional neural network that was used to derive the respective veps.
    ...
    :rtype: numpy.ndarray
    """
    return single_column_kernel(152)(G, V)  # np.matmul(G, V)
...
def phi(func, G, V):
    """Wrapper function for application of kernel function to genotypes.

    i.e. mapping to higher dimensional feature space.
    With V being weights to apply in a certain way to the genotypes.

    :param function func: Which kernel function to use, needs to be a function accepting the arguments G and V
    :param numpy.ndarray G: SNVs to be tested (genotypes), :math:`nxm` (:math:`n:=` number of individuals, :math:`m:=` number of SNVs)
    :param numpy.ndarray V: veps or weights to be used :math:`mxk` (:math:`m:=` number of SNVs, :math:`k:=` number of veps); dimension :math:`m` needs to be equal in G and V
    :return: transformed genotypes (phi(G, V))
    :rtype: numpy.ndarray
    """
    GxV = func(G, V)
    return GxV
...
class LocalCollapsing:
    ...
    def collapse(self, G, pos, weights=1.):
        ...
        """
        Collapses G locally, based on coordinates
